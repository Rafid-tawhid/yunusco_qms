...
import 'package:nidle_qty/widgets/date_picker.dart';
import 'package:nidle_qty/widgets/dhu_file.dart';
...
class _ProductionReportScreenState extends State<ProductionReportScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Quality Control Dashboard'), centerTitle: true, elevation: 1),
              ...
              (context, pro, _) => Column(
                ...
                children: [
                  ...
                  Consumer<CountingProvider>(
                    builder:
                        (context, pro, _) => // In your parent widget:
                            HourlyProductionDashboard(productionData: pro.hourly_production_List),
                  ),
                  Consumer<CountingProvider>(
                    builder:
                        (context, pro, _) => // In your parent widget:
                            SizedBox(height: 500, child: DefectsDisplayWidget(defects: pro.operation_defect)),
                  ),
    ...
    return Card(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [color.withOpacity(0.7), color.withOpacity(0.9)]),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Quality Distribution', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.black87)),
            const SizedBox(height: 8),
            SizedBox(
              height: 300,
              child: SfCartesianChart(
                plotAreaBorderWidth: 0,
                primaryXAxis: CategoryAxis(labelStyle: const TextStyle(color: Colors.black54), axisLine: const AxisLine(width: 0), majorTickLines: const MajorTickLines(size: 0)),
                ...
                series: <CartesianSeries<_ChartData, String>>[
                  ColumnSeries<_ChartData, String>(
                    ...
                    dataLabelSettings: const DataLabelSettings(
                      ...
                      textStyle: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.black87),
                    ),
                    width: 0.6,
                    spacing: 0.2,
                ...
                tooltipBehavior: TooltipBehavior(enable: true, header: 'Category', format: 'point.x : point.y'),
                palette: [Colors.blue.shade400, Colors.green.shade400, Colors.orange.shade400, Colors.red.shade400, Colors.purple.shade400], // Fallback colors if colorMapper not provided
              ),
            ),
          ],
        ),
  ...
  Widget _buildSummaryCards(num total) {
    return Row(
      children: [
        Expanded(child: _StatCard(title: 'Total Items', value: total, icon: Icons.inventory_2_outlined, color: Colors.blueAccent)),
        const SizedBox(width: 12),
        Expanded(child: _StatCard(title: 'Pass Rate', value: total > 0 ? (widget.stats.totalPass ?? 0) / total * 100 : 0, icon: Icons.percent, color: Colors.green, isPercentage: true)),
      ],
    );
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            DataTable(
              columnSpacing: 12,
              columns: const [DataColumn(label: Text('Category')), DataColumn(label: Text('Count')), DataColumn(label: Text('Percentage'))],
              rows:
                  data.map((item) {
                    ...
                    return DataRow(
                      cells: [
                        DataCell(
                          Row(children: [Container(width: 12, height: 12, decoration: BoxDecoration(color: item.color, shape: BoxShape.circle)), const SizedBox(width: 8), Text(item.category)]),
                        ),
...
class _StatCard extends StatelessWidget {
  ...
  const _StatCard({required this.title, required this.value, required this.icon, required this.color, this.isPercentage = false});
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Row(children: [Icon(icon, size: 24, color: color), const SizedBox(width: 8), Text(title, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w500))]),
            const SizedBox(height: 12),
            Text(isPercentage ? '${value.toStringAsFixed(1)}%' : value.toString(), style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: color)),
          ],
        ),
      ),
    );
