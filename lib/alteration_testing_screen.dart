...
class GarmentQualityApp extends StatelessWidget {

  final String form;
  GarmentQualityApp({required this.form});

  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Garment Quality Check',
      debugShowCheckedModeBanner: false,
      theme: ThemeData(
        primarySwatch: Colors.blue,
        visualDensity: VisualDensity.adaptivePlatformDensity,
      ),
      home: const QualityCheckScreen(),
    );
  }
}
...
class _QualityCheckScreenState extends State<QualityCheckScreen> {
  ...
  @override
  Widget build(BuildContext context) {
    final currentOperation = garmentQualityData['operations']!
        .firstWhere((op) => op['id'] == selectedOperationId);
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: _buildReasonsList(reasons),
          ),
  ...
  void _showSubmissionDialog() {
    final currentReasons = garmentQualityData['operations']!
        .firstWhere((op) => op['id'] == selectedOperationId)['reasons'] as List;
    final selectedNames = currentReasons
        .where((reason) => (selectedReasons[selectedOperationId] ?? {}).contains(reason['id']))
        .map<String>((reason) => reason['name'] as String)
        .join(', ');

    debugPrint('SELECTED REASON : ${currentReasons}');
