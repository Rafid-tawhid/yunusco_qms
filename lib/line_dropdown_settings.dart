    ...
    if (sectionName != null) {
      ...
      if (section.isNotEmpty) {
        ...
        await context.read<BuyerProvider>().getAllLinesBySectionId(
          _prefs.getString('selectedSectionId') ?? section['unitID'].toString(),
        );
        ...
        if (lineName != null) {
          ...
          if (line.isNotEmpty) {
            selectedLine = SearchFieldListItem(
              line['name'],
              item: line,
            );
          }
                      ...
                      suggestions: provider.allSection.map((section) {
                        return SearchFieldListItem(
                          ...
                          child: _buildSuggestionItem(
                            section['unitName'],
                            isSelected: selectedSection?.item?['unitName'] ==
                                section['unitName'],
                          ),
                        );
                      }).toList(),
                  ...
                  builder: (context, provider, _) {
                    return SearchField<Map<String, dynamic>>(
                      hint: selectedSection != null
                          ? 'Search line in ${selectedSection!.item!['unitName']}...'
                      ...
                      suggestions: provider.allLines.map((line) {
                        return SearchFieldListItem(
                          line['name'],
                          item: line,
                          child: _buildSuggestionItem(
                            line['name'],
                            isSelected: selectedLine?.item?['name'] == line['name'],
                          ),
                        );
                      }).toList(),
